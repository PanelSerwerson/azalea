...
fn frame_splitter(buffer: &mut BytesMut) -> Result<Option<BytesMut>, FrameSplitterError> {
...
pub async fn read_packet_filtered<P: ProtocolPacket + Debug, R>(
    stream: &mut R,
    buffer: &mut BytesMut,
    compression_threshold: Option<u32>,
    cipher: &mut Option<Aes128CfbDec>,
...
where
    R: AsyncRead + Unpin + Send + Sync,
{
    let raw = read_raw_packet(stream, buffer, compression_threshold, cipher).await?;
    ...
    let packet = deserialize_packet::<P>(&mut Cursor::new(&raw[..]))?;
    Ok(Some(packet))
}
...
where
    R: AsyncRead + std::marker::Unpin + std::marker::Send + std::marker::Sync,
{
    let Some(buf) = frame_splitter(buffer).map_err(ReadPacketError::from)? else {
        return Ok(None);
    };

    let buf = if let Some(compression_threshold) = compression_threshold {
        let decompressed = compression_decoder(&mut Cursor::new(&buf[..]), compression_threshold)
